...
function AddTenantPage() {
    ...
    const vacantRooms = rooms.filter(r => !tenants.some(t => t.room === `Room ${r.number}`))

    const [form, setForm] = useState({
        name: '', room: '', rent: '', phone: '', email: '',
        hometown: '', address: '', mealType: 'vegetarian',
    ...
    const validate = () => {
        ...
        else if (isNaN(form.rent) || Number(form.rent) <= 0) e.rent = 'Enter a valid rent amount'
        ...
        else if (!/^[6-9]\d{9}$/.test(form.emergencyPhone.trim())) e.emergencyPhone = 'Enter a valid 10 digit mobile number'
    ...
    const inputStyle = (field) => ({
        border: errors[field] ? '1px solid var(--danger)' : '1px solid var(--border-color)',
    ...
    const Field = ({ label, name, placeholder, type = 'text' }) => (
        <div className='mb-5'>
            <label className='block text-sm font-semibold mb-2' style={{ color: 'var(--text-primary)' }}>{label}</label>
            <input
                name={name}
                type={type}
                value={form[name]}
        ...
        <PageTransition>
            <div className='flex min-h-screen' style={{ backgroundColor: 'var(--bg-tertiary)' }}>
                ...
                <div className='flex-1 p-4 md:p-8 mt-16 md:mt-0'>
                    <div className='mb-6'>
                        <h1 className='text-2xl md:text-3xl font-bold' style={{ color: 'var(--text-primary)' }}>Add Tenant</h1>
                        <p className='mt-1 text-sm' style={{ color: 'var(--text-secondary)' }}>Fill in all details carefully</p>
                    </div>
                    ...
                    >
                        <h3 className='font-bold text-base mb-4' style={{ color: 'var(--text-primary)' }}>Basic Information</h3>
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-x-6'>
                            <Field label='Full Name *' name='name' placeholder='Paul Allen' />
                            <Field label='Phone Number *' name='phone' placeholder='9876543210' type='tel' />
                            <Field label='Email' name='email' placeholder='paul@gmail.com' type='email' />
                            <Field label='Hometown *' name='hometown' placeholder='Chennai, Tamil Nadu' />
                        </div>

                        <div className='mb-5'>
                            <label className='block text-sm font-semibold mb-2' style={{ color: 'var(--text-primary)' }}>Full Address *</label>
                            <textarea
                                name='address'
                                value={form.address}
                                onChange={handleChange}
                        ...
                        <div className='mb-5'>
                            <label className='block text-sm font-semibold mb-2' style={{ color: 'var(--text-primary)' }}>Meal Preference *</label>
                        ...
                        <h3 className='font-bold text-base mb-4 mt-6' style={{ color: 'var(--text-primary)' }}>Room Details</h3>
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-x-6'>
                            <div className='mb-5'>
                                <label className='block text-sm font-semibold mb-2' style={{ color: 'var(--text-primary)' }}>Select Room *</label>
                                <select
                                    name='room'
                                    value={form.room}
                                    onChange={handleChange}
                                ...
                                {errors.room && <p className='text-xs mt-1' style={{ color: 'var(--danger)' }}>{errors.room}</p>}
                                {vacantRooms.length === 0 && (
                                    <p className='text-xs mt-1' style={{ color: 'var(--danger)' }}>No vacant rooms available</p>
                                )}
                            </div>
                            <div className='mb-5'>
                                <label className='block text-sm font-semibold mb-2' style={{ color: 'var(--text-primary)' }}>Monthly Rent (₹) *</label>
                                <input
                                    name='rent'
                                    value={form.rent}
                                    onChange={handleChange}
                                ...
                                />
                                {errors.rent && <p className='text-xs mt-1' style={{ color: 'var(--danger)' }}>{errors.rent}</p>}
                            </div>
                        </div>

                        <h3 className='font-bold text-base mb-4 mt-2' style={{ color: 'var(--text-primary)' }}>Emergency Contact</h3>
                        <div className='grid grid-cols-1 md:grid-cols-3 gap-x-6'>
                            <Field label='Name *' name='emergencyName' placeholder='Suresh Kumar' />
                            <Field label='Phone *' name='emergencyPhone' placeholder='9845012345' type='tel' />
                            <Field label='Relation *' name='emergencyRelation' placeholder='Father' />
                        </div>
