...
function LoginPage() {
   ...
   const handleLogin = () => {
        fetch('https://pg-manager-backend-mry1.onrender.com/api/tenant-login', {
            method: 'POST',
            ...
            .then(data => {
                if (data.success) {
                    ...
                    sessionStorage.setItem('tenant-id', data.tenantId)
                    toast.success(`Welcome back, ${data.name}!`)
                    navigate('/tenant/dashboard')
                } else {
                    toast.error(data.error || 'Invalid tenant credentials')
                    setError(data.error || 'Invalid tenant credentials')
                }
            })
            .catch(err => {
