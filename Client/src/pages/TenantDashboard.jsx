...
function TenantDashboard() {
  ...
  useEffect(() => {
    if (!tenant) return;
    fetch(`https://pg-manager-backend-mryl.onrender.com/api/documents/${tenant.id}`, {
      credentials: "include",
    })
      .then((res) => res.json())
  ...
  const handleFileUpload = () => {
    ...
    fetch("https://pg-manager-backend-mryl.onrender.com/api/documents", {
      method: "POST",
      credentials: "include",
      body: formData,
    })
  ...
  const handleDeleteDocument = (docId) => {
    fetch(`https://pg-manager-backend-mryl.onrender.com/api/documents/${docId}`, {
      method: "DELETE",
      credentials: "include",
    })
      .then((res) => res.json())
  ...
  const handleSave = () => {
    ...
    fetch(`https://pg-manager-backend-mryl.onrender.com/api/tenants/profile`, {
      method: "PATCH",
      ...
      .then((res) => res.json())
      .then(() => {
        dispatch(updateTenant({ id: tenant.id, ...profileData }));
  ...
  const validateComplaint = () => {
    ...
    if (!complaintForm.description.trim())
      ...
      errors.description = "Description too short, add more detail";
    return errors;
  };
    ...
    return (
      ...
      >
        <p style={{ color: "var(--text-primary)" }}>
          Tenant not found. Please login again.
        </p>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto p-6 md:p-10">
              ...
              {[
                ...
                },
                {
                  ...
                  value: tenant.emergencyContact?.name
                    ? `${tenant.emergencyContact.name} (${tenant.emergencyContact.relation}) — ${tenant.emergencyContact.phone}`
                    : "—",
                },
              ].map((item) => (
                <div
          ...
          {documents.length === 0 ? (
            ...
            <ul className="space-y-2">
              {documents.map((doc) => (
                ...
                >
                  <a
                    href={`https://pg-manager-backend-mryl.onrender.com/uploads/${doc.filename}`}
                    target="_blank"
                    rel="noreferrer"
              ...
              <tbody>
                {tenant.rentHistory.map((entry, index) => (
                  <tr
                    ...
                    style={{
                      backgroundColor:
                        index % 2 === 0 ? "var(--bg-card)" : "var(--bg-input)",
        ...
        >
          ...
          <p
            ...
            style={{ color: "var(--text-secondary)" }}
          >
            Let us know about any issue and we'll get it sorted
          </p>
          ...
          <div className="mb-5">
            ...
            <textarea
              ...
              style={{
                border: complaintErrors.description
